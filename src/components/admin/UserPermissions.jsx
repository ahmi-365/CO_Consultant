...
import { Search, UserPlus, X, Users, Info } from "lucide-react";
                          ...
                          <div className="flex flex-wrap gap-1 mt-1">
                            {/* {userGroup.permissions && userGroup.permissions.slice(0, 3).map((permission, index) => (
                              <Badge 
                                key={index}
                                variant="secondary" 
                                className={`text-xs ${getPermissionColor ? getPermissionColor(permission.permission) : ""}`}
                              >
                                {PERMISSION_OPTIONS ? 
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedUserDetails.permissions?.map((permission, index) => (
                    <div key={index} className="flex items-center gap-2">
                      <Badge 
                        variant="secondary" 
                        className={getPermissionColor ? getPermissionColor(permission.permission) : ""}
                      >
                        {PERMISSION_OPTIONS ? 
                          (PERMISSION_OPTIONS.find(p => p.value === permission.permission)?.label || permission.permission) :
                          permission.permission
