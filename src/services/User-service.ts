export const userService = {
  // ✅ Get all users
  async getAll(): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
...
console.log("Fetched users:", result);
  ...
  // ✅ Create user
  async create(data: { name: string; email: string; password: string; role?: string }): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      const token = localStorage.getItem("token");
  ...
  // ✅ Update user
  async update(id: number, data: Partial<{ name: string; email: string; password: string; role: string }>): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      const token = localStorage.getItem("token");
